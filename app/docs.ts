		...
		fragment(html`
			<div class="content">
				...
				<p>
					...
					approaches serve the same purpose of structuring and rendering your HTML elements but offer different
					syntax options. You can choose the one that best fits your coding style and project requirements.
				</p>
				...
				<p>
					...
					<code>onConnected$()</code> function. When a function has <code>$</code> at the end of it's name, that
					means that function is being binded to a <code>Node</code>'s lifecycle.
				</p>
				<x ${Codeblock(await import("./snippets/lifecycle?raw").then((m) => m.default))}></x>

				<x ${Heading(h3(), "signals")}>Signals</x>
				<p>
					In the world of <strong>master-ts</strong>, reactivity is made possible through the seamless integration
					of signals. Signals serve a dual purpose: they allow for the observation of value changes and enable
				...
				<p>
					After following a signal manually, if you don't wanna follow the signal until the App exits, you have to
					unfollow it manually
				</p>
