...
export function createDerive<T>(deriver: SignalDeriver<T>, staticDependencies?: SignalReadable<any>[]): SignalReadable<T> {
	...
	if (staticDependencies) {
		...
		activate = (set: SignalSetter<T>) => {
			console.log("%cactivating static derive", "color:green", self.id, self.value, Array.from(staticDependencies))
			function update() {
		...
		deactivate = () => {
			console.log("%cdeactivating static derive", "color:green", self.id, self.value, Array.from(staticDependencies))
		...
		activate = (set: SignalSetter<T>) => {
			console.log("%cactivating derive", "color:green", self.id, self.value, Array.from(dependencyToSubscriptionMap))
			function update() {
				signalSyncContextStack.push(new Set())
				const value = deriver()
		...
		deactivate = () => {
			console.log("%cdeactivating derive", "color:green", self.id, self.value, Array.from(dependencyToSubscriptionMap))
