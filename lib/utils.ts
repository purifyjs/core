...
export type Equal<T, U> = T extends U ? U extends T ? true
    : false
    : false;
...
export type IsFunction<T, TReturns = any, TArgs = any> = Fn extends T
    ? T extends (...args: infer Args) => infer R ? true extends Equal<R, TReturns> & Equal<Args, TArgs> ? true
        : false
    : false
    : false;
...
export let instancesOf = <T extends abstract new (...args: never) => unknown>(target: unknown, constructor: T): target is InstanceType<T> =>
